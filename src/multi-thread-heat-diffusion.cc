...
void init_grid_values(float * a, float value, int start, int end) {
    for (int i = start; i < end; ++i){
        a[i] = value;
    }
}

void copy_array_elements(float * lhs, float * rhs, int start, int end) {
    for (int i = start; i < end; ++i) {
        lhs[i] = rhs[i];
    }
}
...
void mono_3d (float * old_grid, float * new_grid, int width, float k, int area, int size, int start, int end) {
...
void mono_2d (float * old_grid, float * new_grid, int width, float k, int area, int start, int end) {
    for (int idx = start; idx < end; ++idx) {
        float oldValue = old_grid[idx];
        float * newValueLoc = &new_grid[idx];
        if (idx % width != 0) {
...
void thread_copy_array(int num_threads, int chunk_size, std::vector<std::thread> & threads, float * new_grid, float * old_grid, int size) {
    for (int thread_num = 0; thread_num < num_threads - 1; ++thread_num) {
        threads[thread_num] = std::thread(copy_array_elements, old_grid, new_grid,
                thread_num * chunk_size, (thread_num + 1) * chunk_size);
    }
    threads.back() = std::thread(copy_array_elements, old_grid, new_grid,
                 (num_threads - 1) * chunk_size, size);
...
void thread_init_array(int num_threads, int chunk_size, std::vector<std::thread> & threads, float * new_grid, float value, int size) {
    for (int thread_num = 0; thread_num < num_threads - 1; ++thread_num) {
        threads[thread_num] = std::thread(init_grid_values, new_grid, value,
                thread_num * chunk_size, (thread_num + 1) * chunk_size);
    }
    threads.back() = std::thread(init_grid_values, new_grid, value,
                 (num_threads - 1) * chunk_size, size);
...
void thread_mono_2d(int num_threads, int chunk_size, std::vector<std::thread> & threads, float * new_grid, float * old_grid, int width, float k, int area, int size) {
    for (int thread_num = 0; thread_num < num_threads - 1; ++thread_num) {
        threads[thread_num] = std::thread(mono_2d, old_grid, new_grid, width, k, area,
                thread_num * chunk_size, (thread_num + 1) * chunk_size);
    }
    threads.back() = std::thread(mono_2d, old_grid, new_grid, width, k, area,
                 (num_threads - 1) * chunk_size, size);
...
void thread_mono_3d(int num_threads, int chunk_size, std::vector<std::thread> & threads, float * new_grid, float * old_grid, int width, float k, int area, int size) {
    for (int thread_num = 0; thread_num < num_threads - 1; ++thread_num) {
        threads[thread_num] = std::thread(mono_3d, old_grid, new_grid, width, k, area, size,
                thread_num * chunk_size, (thread_num + 1) * chunk_size);
    }
    threads.back() = std::thread(mono_3d, old_grid, new_grid, width, k, area, size,
                 (num_threads - 1) * chunk_size, size);
...
int main(int argc, char * argv[]) {
    ...
    thread_init_array(num_threads, chunk_size, threads, new_grid, conf.starting_temp, size);
    copy_fixed_blocks(conf, 0, new_grid);

    if (conf.is_3d) {
        for (int i = 0; i < conf.num_timesteps; ++i) { 
            thread_copy_array(num_threads, chunk_size, threads, new_grid, old_grid, size);
            thread_mono_3d(num_threads, chunk_size, threads, new_grid, old_grid, conf.grid_width,
                conf.k, area, size);
            copy_fixed_blocks(conf, 0, new_grid);
        ...
        for (int i = 0; i < conf.num_timesteps; ++i) { 
            thread_copy_array(num_threads, chunk_size, threads, new_grid, old_grid, size);
            thread_mono_2d(num_threads, chunk_size, threads, new_grid, old_grid, conf.grid_width,
                 conf.k, area, size);
            copy_fixed_blocks(conf, 0, new_grid);
        }
    }
